...
const int buzzerPin = 2;  // D2 corresponds to GPIO 2 on ESP32
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    if (registerMode) {
      ...
      if (!isCardRegistered(mfrc522.uid.uidByte, mfrc522.uid.size)) {
        if (registeredCount < MAX_CARDS) {
          if (inputUsername.length() > 0) {
            ...
            buzz(1000);  // Buzz for 1 second for successful registration
            delay(2000);
      ...
      digitalWrite(ledPin3, HIGH);  // Turn on LED3
       buzz(1000);  // Buzz for 1 second for successful removal
      ...
      if (isCardRegistered(mfrc522.uid.uidByte, mfrc522.uid.size)) {
        ...
        buzz(500);  // Buzz for 500ms for successful access
        unlockDoor();
        digitalWrite(ledPin1, LOW);
        ...
        buzz(1000);  // Buzz for 1 second for access denied
        delay(5000);
        digitalWrite(ledPin2, LOW);
